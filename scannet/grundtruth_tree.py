...
# ----- data direction  -----
# --------------------------------------
output_folder = '/media/rubing/hdd/data_label/IKG_hdf5_test_grundtruth_tree_Withoutsubsample'
...
def room2blocks(data, label,num_point):
    ...
    while len(queue):
        ...
        if level >4:
            break
    #
    # data_batch, label_batch = np.array(block_data_list), np.array(block_label_list)
    # np.expand_dims(data_batch, 1)
    # xx = np.concatenate(data_batch)
    return np.concatenate(block_data_list, 0), np.concatenate(block_label_list, 0)#block_data_list, block_label_list
...
def sample_data_label(data, label, num_sample):
    ...
    if N >= num_sample:
        ...
        num = int(np.ceil(float(num_sample) / N))
        for num in range(num):
            start_idx = num * N
            end_idx = (num + 1) * N
            if end_idx <= num_sample-1:
                ...
                data_temp = data[0:num_sample - start_idx-1]
                label_temp = label[0:num_sample - start_idx-1]
                datas.append((data_temp))
                labels.append(label_temp)
        datas = np.expand_dims(np.expand_dims(np.concatenate(datas, 0), 0), 0)
        labels = np.expand_dims(np.expand_dims(np.concatenate(labels, 0), 0), 0)
